...
<%
	...
	SQL="Select * from xin_Index_Type where id="&request("id")
...
if Request("Action")=1 then
	...
	rs.Close
	Set rs=nothing
	Response.Write "<script language='javascript'>alert('修改成功!');document.location.href('New_Type_Manage.asp');</script>"
	Response.End()
else
%>
    ...
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" class="ta">
      <tr>
	   ...
	   <%if rs("pid")=0 then 
			...
			sql="Select id,name from xin_Index_type where id="&rs("pid")
		...
		sql="Select id,name from xin_Index_type where pid=0 order by ord asc"
		Set t=conn.execute(sql)
		do while not t.eof
