...
if Request("pid")="" then
	...
	Set t=conn.execute("Select name from xin_Index_type where id="&Request("pid"))
	if not t.eof then
		n=t(0)
		pid=Request("pid")
	else
...
if Request("Action")=1 then
	...
	SQL="Select * from xin_Index_Type where name='"&Trim(Request("name"))&"'"
	...
	rs.Close
	Set rs=nothing
	Response.Write "<script language='javascript'>alert('您成功添加了一个栏目!');document.location.href('New_Type_Manage.asp');</script>"
	Response.End()
else
%>
