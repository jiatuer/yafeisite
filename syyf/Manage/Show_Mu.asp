...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td>
	<%
	if Request("tid")="" or Request("tid")="error" then
	Response.Write "<script language='javascript'>alert('请选择栏目！');history.go(-1);</script>"
	Response.End()
	end if
	sql="select * from xin_Show where tid="&request("tid")&" order by id desc"
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="1" class="ta">
            ...
            <td height="25" colspan="3" bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			 ...
			 %>
	          <tr>
				...
				tid=rs("tid")
				sql1="select * from xin_Show_Info where id="&tid
				...
				%></td>
                <td width="30%" height="25" align="center" valign="middle"><a href="Show_Edit.asp?id=<%=rs("id")%>">修改</a>| <a href="News_Del.asp?id=<%=rs("id")%>" ONCLICK="javascript:return confirm('提示：您确定要继续执行该操作吗？')">删除</a></td>
              </tr>
			  <%
			  pages = pages - 1
			  rs.movenext
	  ...
	  <form name="form1" method="post" action="Show_Mu.asp">
        ...
        <td height="30" align="right" valign="middle" class="tb"><a href="?page=1&tid=<%=request("tid")%>">首页</a>&nbsp; <a href="?page=<%=page-1%>&tid=<%=request("tid")%>">上一页</a> &nbsp; <a href="?page=<%=page+1%>&tid=<%=request("tid")%>">下一页</a>&nbsp; <a href="?page=<%=allpages%>&tid=<%=request("tid")%>">尾页</a> &nbsp;
