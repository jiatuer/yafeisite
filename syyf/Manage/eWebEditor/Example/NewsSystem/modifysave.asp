...
' ======================
' 功能：修改文章保存页
' 描述：对modify.asp文件提交过来的表单数据进行保存操作，有保存文章标题，文章内容，文章标题图片；
'       同时保存所有此篇文章所有相关的上传或远程获取的文件信息，有源文件名，保存文件名，保存路径文件名。
' ======================
...
Sub Content()
	...
	' 文章ID有效性验证，防止有些人恶意的破坏此演示程序
	If IsNumeric(sNewsID) = False Then
		GoError "请通过页面上的链接进行操作，不要试图破坏此演示系统。"
	End If

	' 取提交过来的数据
	' 注意取文章内容的方法，因为对大表单的自动处理，一定要使用循环，否则大于100K的内容将取不到，单个表单项的限制为102399字节（100K左右）
	...
	If Not oRs.Eof Then
		...
		oRs.Update
	Else
		GoError "无效的文章ID，请点页面上的链接进行操作！"
	End If
	oRs.Close

	' 输出成功保存信息
	Response.Write "文章（ID：" & sNewsID & "）修改保存成功！"
