...
<%
  ...
  Select Case Action
    ...
    Case 2                         ' 下拉菜单添加
      ...
      End if
      Set Rs=Server.CreateObject("ADODB.Recordset")
      Sql = "select * from xin_Types where Type_value = '"&Title&"'"
      Rs.open sql,conn,1,3
      If not Rs.eof then
      ...
      Else
        ...
        Rs.close     
        Set rs = nothing
        response.write"<script language=JavaScript>{window.alert('您成功的添加了一条下拉菜单');window.location.href='Admin_Menu.asp?action=1'}</script>"
      End if      
    Case 3                         ' 下拉菜单修改
      id = request("id")
      Title = request("Title")
      ...
      End if
      Set Rs=Server.CreateObject("ADODB.Recordset")
      Sql = "select * from xin_Types where Type_id = "&id
      Rs.open sql,conn,1,3
      ...
      Else
        ...
        Rs.close     
        Set rs = nothing
        response.write"<script language=JavaScript>{window.alert('您成功的修改了一条下拉菜单');window.location.href='Admin_Menu.asp?action=1'}</script>"
      End if    
    Case 4                         ' 下拉菜单删除
      Action_id = request("id")
      Set rs=Server.CreateObject("ADODB.Recordset")      
      Sql="Delete * from xin_Types where Type_id="&Action_id
      Rs.Open Sql,conn,1,1          
      response.write"<script language=JavaScript>{window.alert('您已经成功删除一条记录');window.location.href='Admin_Menu.asp?Action=1'}</script>"
      Set Rs = nothing
    Case 5                         ' 下拉菜单全部删除
      Set rs=Server.CreateObject("ADODB.Recordset")      
      Sql="Delete * from xin_Types"
      Rs.Open Sql,conn,1,1          
      response.write"<script language=JavaScript>{window.alert('您已经成功全部记录');window.location.href='Admin_Menu.asp?Action=1'}</script>"
      Set rs = nothing  
    Case 6                         ' 反馈信息全部删除
      fs = request("fs")
      If fs = "Mail" Then 
